...
class UNet(MonaiUNet):
    def __init__(self, cfg: DictConfig | Dict[str, Any], in_channels: Optional[int] = None, eps: Optional[float] = None):
        ...
        c_in = in_channels if in_channels is not None else (None if c_in_cfg == "auto" else int(c_in_cfg))
        if c_in is None:
            raise ValueError("[UNet] in_channels is 'auto'; please pass in_channels at construction time.")
        ...
        log.info(f"[Gen] UNet: in={c_in}, out={out_ch}, channels={channels}, strides={strides}, "
                 f"res_units={num_res_units}, act={act}, norm={norm}, dropout={dropout}")
